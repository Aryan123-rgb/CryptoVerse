...
function TableComponent() {
  ...
  if (cryptoData) {
    filteredData = cryptoData.filter(
      (item) => item.name === "Bitcoin" || item.symbol === "btc"
    );
  }
  ...
  return (
    <>
      <div className="border-gray-800 border rounded-lg flex flex-col mt-9 shadow-lg overflow-scroll table-class">
        {cryptoData ? (
          <table className="w-full table-auto divide-y divide-gray-950 overflow-x-scroll overflow-scroll">
            <thead className="capitalize text-base">
              <tr>
                ...
                <th className="py-1 font-sans">1H</th>
                <th className="py-1 font-sans">24H</th>
                <th className="py-1 font-sans">7D</th>
              </tr>
            </thead>
            <tbody>
              {cryptoData.map((data) => {
                return (
                  <tr
                    key={data.id}
                    className="text-center text-base border-b border-gray-950 hover:bg-zinc-200 last:border-b-0"
                    onClick={()=>navigate(`/crypto/${data.id}`)}
                  >
                    <td className="py-4 flex items-center uppercase">
                      <button className="outline-0 border-0 bg-none cursor-pointer">
                        <svg
                          className="w-[1.5rem] ml-1.5 hover:fill-blue-500"
                          width="30"
                          height="30"
                          viewBox="0 0 30 30"
                          fill="gray-600"
                    ...
                    <td
                      className={
                        data.price_change_percentage_1h_in_currency > 0
                          ? "py-4 font-sans text-green-500 font-bold"
                          : "py-4 font-sans text-red-600 font-bold"
                      }
                    >
                      {Number(
      ...
      <div className="flex items-center justify-between mt-4 capitalize h-[2rem]">
        <span>Inspired by <a className="text-blue-600" href="https://www.coingecko.com" target="_blank">Coin Gecko</a></span>
        <Paginations />
      </div>
    </>
  );
